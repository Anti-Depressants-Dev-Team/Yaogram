...
QByteArray Settings::serialize() const {
	...
	result.reserve(size);
	{
		...
		stream
			...
			<< qint32(_screenCaptureProtection.current() ? 1 : 0)
...
void Settings::addFromSerialized(const QByteArray &serialized) {
	...
	qint32 screenCaptureProtection = _screenCaptureProtection.current() ? 1 : 0;
	...
	if (!stream.atEnd()) {
		stream >> screenCaptureProtection;
	}
	if (!stream.atEnd()) {
	...
	_screenCaptureProtection = (screenCaptureProtection == 1);
