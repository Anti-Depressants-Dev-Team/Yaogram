...
void BuildSecuritySection(
	...
	builder.add([](const WidgetContext &ctx) {
		const auto button = ctx.container->add(object_ptr<Ui::SettingsButton>(
			ctx.container,
			rpl::single(QString("Screen Capture Protection")),
			st::settingsButtonNoIcon));
		button->toggleOn(
			Core::App().settings().screenCaptureProtectionValue()
		)->toggledChanges(
		) | rpl::on_next([](bool toggled) {
			Core::App().settings().setScreenCaptureProtection(toggled);
			Core::App().saveSettingsDelayed();
		}, button->lifetime());
		return SectionBuilder::WidgetToAdd{ button };
	});
